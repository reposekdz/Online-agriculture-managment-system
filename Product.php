...
if(isset($_POST['submit']))
{
	...
	if(isset($_GET['editid']))
	{
		...
		$sql = $sql . " quantity='$_POST[quantity]', quantity_type='$_POST[quantitytype]', uploaded_date='$dt', status='$status' WHERE product_id='$_GET[editid]'";
	...
	else
	{
		...
		$sql = $sql . " , quantity, quantity_type, uploaded_date, status) VALUES ('','$_SESSION[sellerid]','$_POST[category]','$_POST[produce]','$_POST[variety]','$_POST[title]','$imgname1'";
		if($_FILES['img2']['name'] != "")
		{
		$sql = $sql . " ,'$imgname2'";
		}
		$sql = $sql . " ,'$_POST[quantity]','$_POST[quantitytype]','$dt','$status')";
		if(!mysqli_query($con,$sql))
		{
			echo "Error in mysqli query";
		}
		else
		{
			...
			{
			echo "<script>alert('Your Product Has Been Put On Sale...');</script>";
			}
			else
			{
...
<div class="form-row">
	...
	Quantity Type <font color="#FF0000">*</font>
	 <select name="quantitytype" id="quantitytype" class="form-control">
                                 <option value="">Select</option>
                                  <?php
								  $arr= array("Kilogram","Quintal","Piece");
								  foreach($arr as $val)
								  {
